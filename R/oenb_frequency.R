...
oenb_frequency <- function(id, pos, lang = "EN") {
  ...
  # This function exists to show which frequencies can be chosen when querying
  # the series. The service reports a frequency once per combination of the
  # attributes, so the same choice arrives many times over. Each choice is
  # listed once, as oenb_attributes lists the values of an attribute once.
